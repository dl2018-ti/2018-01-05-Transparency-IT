...
class OpportunitiesController extends AppController
{
    ...
    public function front_listing()
    {
        $this->paginate = [
            'contain' => ['OpportunityTypes', 'Countries', 'States', 'OpportunityStatuses', 'Industries', 'SalaryTypes', 'WorkTypes'],
            'conditions' => ['Opportunities.end_date >=' => date("Y-m-d")],
    ...
    public function search()
    {
        ...
        if( $data['job_query'] != "" && $data['job_location'] > 0 ){
            $query_search       = $data['job_query'];
            $query_job_location = $data['job_location'];
        ...
        $states    = $this->Opportunities->States->find('list');     
        $this->viewBuilder()->layout("job_list");    
        $this->set(['countries' => $countries, 'states' => $states, 'query_job_location' => $query_job_location, 'query_search' => $query_search]);
    ...
    {           
        ...
        if($this->request->is('post')){
            ...
            $email->from(['websystem@transparencyit.com' => 'Transparency IT System'])
